...
class Posts extends CI_Controller {
	...
	public function index($id=0,$seo_url='')
	{
		...
		$page_title = $this->lang->line('posts');
			...
			if( $this->init->Have_Permission('posts') )
			{
				
				$query = $this->db->get_where('tblposts', array('id' => $id , 'status' => 'Active'));
				...
				foreach ($query->result() as $row)
				{
					...
					$query2 = $this->db->get_where('tblcategories', array('id' => $row->cat_id));
					$row2 = $query2->row();
					...
					# Load SHOW USERS Template
					$this->load->view($this->init->Site_Template.'/posts', $data); 	
