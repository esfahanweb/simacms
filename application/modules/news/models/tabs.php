...
class tabs extends CI_Model {
	...
	function tab_menu($id, $active)
	{
		...
		foreach($array as $row)
		{
			if($row['title'] == $active)
			{
				$output .= '<li style="background-color: #fafafa;"><a href="'.base_url().'admin/users/profiles/'.$row['title'].'/'.$id.'">'.$row['lang'].'</a></li>';
			}
			else
			{
				$output .= '<li ><a href="'.base_url().'admin/users/profiles/'.$row['title'].'/'.$id.'">'.$row['lang'].'</a></li>';
			}
		}
		return $output;
	}
