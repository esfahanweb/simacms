...
require_once 'xmlrpc.inc';
  class IBSngAdapter
  {
    ...
    function load($server_ip, $server_port, $timeout, $auth_name, $auth_pass)
    {
	...
	function getOnlineUsers($normal_sort_by, $normal_desc, $voip_sort_by, $voip_desc, $conds)
	{
		...
		$params_arr = array('normal_sort_by' => $normal_sort_by, 'normal_desc' => $normal_desc, 'voip_sort_by' => $voip_sort_by, 'voip_desc' => $voip_desc, 'conds' => $conds);
     	$server_method = 'report.getOnlineUsers';
	...
	function listGroups()
	{
		...
		//$params_arr = array('normal_sort_by' => $normal_sort_by, 'normal_desc' => $normal_desc, 'voip_sort_by' => $voip_sort_by, 'voip_desc' => $voip_desc, 'conds' => $conds);
     	$server_method = 'group.listGroups';
      	list($success, $resp) = $this->rpc_request($server_method, $params_arr);
      	if(!$success)
      	{
        	return array(0, array());
	...
	function editUserAttrByUserName($normal_username, $attrs, $to_del_attrs = array())
    {
	  $user_id = strval($this->getUserId($normal_username));
