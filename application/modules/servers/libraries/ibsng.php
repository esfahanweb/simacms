 ...
 */
    ...
    function ibsng_configoptions ()
  	{
			...
			'Shell Access' => array ('Type' => 'yesno', 
			'Description' => 'Tick to grant access'), 
			'Max SQL Databases' => array ('Type' => 'text', 'Size' => '5'), 
			'CGI Access' => array ('Type' => 'yesno', 'Description' => 'Tick to grant access'), 
			'Max Subdomains' => array ('Type' => 'text', 'Size' => '5'), 
			'Frontpage Extensions' => array ('Type' => 'yesno', 'Description' => 'Tick to grant access'), 
			'Max Parked Domains' => array ('Type' => 'text', 'Size' => '5'), 
			'cPanel Theme' => array ('Type' => 'text', 'Size' => '15'), 
			'Max Addon Domains' => array ('Type' => 'text', 'Size' => '5'), 
			'Limit Reseller by Number' => array ('Type' => 'text', 'Size' => '5', 'Description' => 'Enter max number of allowed accounts'), 
			'Limit Reseller by Usage' => array ('Type' => 'yesno', 'Description' => 'Tick to limit by resource usage'), 
			'Disk Space' => array ('Type' => 'text', 'Size' => '7', 'Description' => 'MB'), 
			'Bandwidth' => array ('Type' => 'text', 'Size' => '7', 'Description' => 'MB'), 
			'Allow DS Overselling' => array ('Type' => 'yesno', 'Description' => 'MB'), 
			'Allow BW Overselling' => array ('Type' => 'yesno', 'Description' => 'MB'), 
			'Reseller ACL List' => array ('Type' => 'text', 'Size' => '20')
			);
  ...
  function ibsng_clientarea ($params)
  {
    ...
    {
      $code = '<form method="post" action="' . $http . '://' . $params['serverip'] . ':' . $ibsngport . '/login/" method="post" target="_blank">
		<input type="hidden" name="user" value="' . $params['username'] . '">
		<input type="hidden" name="pass" value="' . $params['password'] . '">
		<input type="submit" value="' . $_LANG['clientareaibsnglink'] . '" class="button">
  ...
  function adminlink ($params)
  {
   	 ...
   	 $code = '<form id="whm" action="' . $http . '://' . $params['ipaddress'] . '/IBSng/admin/" method="post" target="_blank"><input type="hidden" name="user" value="' . $params['username'] . '"><input type="hidden" name="pass" value="' . $params['password'] . '"><input id="whm" type="submit" value="WHM"></form>';
  ...
  function ibsng_createaccount ($params)
  {
    ...
    if ($params['type'] == 'reselleraccount')
    {
      $ibsngrequest = '/scripts/addres?res=' . urlencode ($params['username']) . '&cowner=1';
      $output = ibsng_req ($params['serversecure'], $params['serverip'], $params['serverusername'], $params['serverpassword'], $params['serveraccesshash'], $ibsngrequest);
      $ibsngrequest = '/scripts2/editressv?res=' . urlencode ($params['username']);
      if ($params['configoption16'])
      {
        $ibsngrequest .= '&limits_resources=1&rslimit-disk=' . urlencode ($params['configoption17']) . '&rslimit-bw=' . urlencode ($params['configoption18']);
        if ($params['configoption19'])
        {
          $ibsngrequest .= '&rsolimit-disk=1';
        }

        if ($params['configoption20'])
        {
          $ibsngrequest .= '&rsolimit-bw=1';
        }
      }
      else
      {
        $ibsngrequest .= '&limits_number_of_accounts=1&1resnumlimitamt=' . urlencode ($params['configoption15']);
      }

      $output = ibsng_req ($params['serversecure'], $params['serverip'], $params['serverusername'], $params['serverpassword'], $params['serveraccesshash'], $ibsngrequest);
      $ibsngrequest = '/xml-api/setacls?reseller=' . urlencode ($params['username']) . '&acllist=' . urlencode ($params['configoption21']);
      $output = ibsng_req ($params['serversecure'], $params['serverip'], $params['serverusername'], $params['serverpassword'], $params['serveraccesshash'], $ibsngrequest);
    }
  ...
  function ibsng_terminateaccount ($params)
  {
    ...
    $ibsngrequest = '/scripts/killacct?domain=' . $params['username'] . '&user=' . $params['username'] . '';
    $output = ibsng_req ($params['serversecure'], $params['serverip'], $params['serverusername'], $params['serverpassword'], $params['serveraccesshash'], $ibsngrequest);
    if (strpos ($output, 'Curl Error') == true)
    {
      $result = $output;
  ...
  function ibsng_suspendaccount ($params)
  {
    ...
    if ($params['type'] == 'reselleraccount')
    {
      $ibsngrequest = '/scripts/suspendreseller?reseller=' . $params['username'] . '&resalso=1';
    }
    else
    {
      $ibsngrequest = '/scripts2/suspendacct?domain=' . $params['username'] . '&user=' . $params['username'] . '&suspend-domain=Suspend';
    }
  ...
  function ibsng_unsuspendaccount ($params)
  {
    ...
    if ($params['type'] == 'reselleraccount')
    {
      $ibsngrequest = '/scripts/suspendreseller?reseller=' . $params['username'] . '&resalso=1&un=1';
    }
    else
    {
      $ibsngrequest = '/scripts2/suspendacct?domain=' . $params['username'] . '&user=' . $params['username'] . '&unsuspend-domain=UnSuspend';
    }
  ...
  function ibsng_changepassword ($params)
  {
    ...
    $ibsngrequest = '/scripts/passwd?password=' . $params['password'] . '&user=' . $params['username'] . '';
    $output = ibsng_req ($params['serversecure'], $params['serverip'], $params['serverusername'], $params['serverpassword'], $params['serveraccesshash'], $ibsngrequest);
    if (strpos ($output, 'Curl Error') == true)
    {
      $result = $output;
  ...
  function ibsng_changepackage ($params)
  {
    if (!$params['username'])
    {
      return 'Cannot perform action without accounts username';
    ...
    $ibsngrequest = '/scripts2/upacct?user=' . $params['username'] . '&pkg=' . urlencode ($params['configoption1']) . '';
    $output = ibsng_req ($params['serversecure'], $params['serverip'], $params['serverusername'], $params['serverpassword'], $params['serveraccesshash'], $ibsngrequest);
    if (strpos ($output, 'Curl Error') == true)
    {
      $result = $output;
  ...
  function ibsng_loginlink ($params)
  {
    if ($params['serversecure'])
    {
      $whmport = '2087';
    ...
    echo '<a href="' . $http . '://' . $params['serverip'] . ':' . $whmport . '/xferibsng/' . $params['username'] . '" target="_blank" style="color:#cc0000">login to control panel</a>';
  }

  function ibsng_usageupdate ($params)
  {
    $ibsngrequest = '/xml-api/listaccts';
    $output = ibsng_req ($params['serversecure'], $params['serverip'], $params['serverusername'], $params['serverpassword'], $params['serveraccesshash'], $ibsngrequest);
  ...
  function ibsng_req ($usessl, $host, $user, $pass, $accesshash, $request)
  {
    global $debug_output;
