...
class Details extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('details_m', 'details');    
    }
	...
	public function show()
	{
		...
		$page_title = $this->lang->line('show_search_discounts');
		...
		if( $this->admin_init->Admin_Logged_In() )
		{
			...
			{	
				$data['show_details'] = $this->details->show_search();
					
				# Load SHOW USERS Template
				$this->load->view('users/details/show', $data);
			}
			else
			{
				# Show Error
	...
	public function edit($id)
	{
		...
		if( $this->admin_init->Admin_Logged_In() )
		{
			...
			if( $this->admin_init->Have_Permission() )
			{
				...
				if ($this->form_validation->run() == FALSE)
				{
					$this->details->edit_form($id);
				}
				# Run If Data Sent By Form
				else
				{	
					$this->details->update($id, $_POST);
