...
class Profiles_m extends CI_Model {
	...
	{				
		...
		if($query->num_rows() > 0)
		{
			...
			$this->load->view('admin/profiles/summary', $users); 	
		}
		else
		{
			# Show Error
	...
	function register()
	{
		$users['site_language'] = form_dropdown('site_language', $this->language_options, $this->init->User['site_language'] );
		$users['site_template'] = form_dropdown('site_template', $this->site_template_options, $this->init->User['site_template'] );	
		return $users;		
	}
	...
	function edit($id = '')
	{
		if($id == '')
		{
			...
			$users['group'] = $this->init->Group['title'];		
		}
		else
		{
			$query = $this->db->get_where('users', array('id' => $id));
			$users = $query->row_array();
			# Get user group data
			$Group = $this->init->Fetch_Table_Fields('groups', $users['group_id']);
			if($Group['type'] == 'admin')
			{
				$users['admin_language'] = form_dropdown('admin_language', $this->language_options, $users['admin_language'] );
				$users['admin_template'] = form_dropdown('admin_template',$this->admin_template_options, $users['admin_template'] );
	...
	{				
		...
		if($query->num_rows() > 0)
		{
			...
			$this->load->view('admin/profiles/delete', $users); 	
		}
		else
		{
			# Show Error
	...
	{	
		...
		# Show Success Page
		$this->admin_init->Show_Success_Page($this->lang->line('delete_user_success'),base_url().'admin/users/profiles/show');	
	}
	...
	function invoices($id)
	{
		...
		$this->load->view('admin/profiles/invoices', $users);
	}	
}