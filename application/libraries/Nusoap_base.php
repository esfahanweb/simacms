...
// necessary classes
require_once('class.soapUser.php');
...
class nusoap_fault extends nusoap_base {
	...
	* constructor
    *
    * @param string $faultcode (SOAP-ENV:User | SOAP-ENV:Server)
...
//		}
		...
		if ($this->scheme == 'https') {
			...
			// support User certificates (thanks Tobias Boes, Doug Anarino, Eryan Ariobowo)
...
//		}
		...
		if (
			...
			$this->setError("Unsupported HTTP response status $http_status $http_reason (soapUser->response has contents of the response)");
			return false;
		}
...
class nusoap_server extends nusoap_base {
			...
			if ($this->headers['content-encoding'] == 'deflate' || $this->headers['content-encoding'] == 'gzip') {
				...
				if (function_exists('gzuncompress')) {
					if ($this->headers['content-encoding'] == 'deflate' && $degzdata = @gzuncompress($data)) {
						...
						$this->fault('SOAP-ENV:User', 'Errors occurred when trying to decode the data');
						return;
					}
				} else {
					$this->fault('SOAP-ENV:User', 'This Server does not support compressed data');
					return;
				}
			}
		}
	...
	* fields to affect the response sent to the User
	*
	* responseHeaders
	* outgoing_headers
	*
	...
	*/
	function invoke_method() {
		...
		// you use SOAP RPC, and the User is .NET, please be aware of the following...
		...
		if ($this->wsdl) {
			if ($this->opData = $this->wsdl->getOperationData($this->methodname)) {
				...
				$this->fault('SOAP-ENV:User', "Operation '" . $this->methodname . "' is not defined in the WSDL for this service");
				return;
			}
		} else {
		...
		if ($class == '') {
			if (!function_exists($this->methodname)) {
				...
				$this->fault('SOAP-ENV:User',"method '$this->methodname'('$orig_methodname') not defined in service('$try_class' '$delim')");
				return;
			}
		} else {
			...
			if (!in_array($method_to_compare, get_class_methods($class))) {
				...
				$this->result = 'fault: method not found';
				$this->fault('SOAP-ENV:User',"method '$this->methodname'/'$method_to_compare'('$orig_methodname') not defined in service/'$class'('$try_class' '$delim')");
				return;
			}
		}
		...
		if(! $this->verify_method($this->methodname,$this->methodparams)){
			...
			// return fault
			$this->fault('SOAP-ENV:User',"Operation '$this->methodname' not defined in service.");
			return;
		}
			...
			if ($this->methodparams) {
				foreach ($this->methodparams as $param) {
					if (is_array($param) || is_object($param)) {
						$this->fault('SOAP-ENV:User', 'NuSOAP does not handle complexType parameters correctly when using eval; call_user_func_array must be available');
						return;
					}
					$funcCall .= "\"$param\",";
				}
	...
	*/
	...
	/**
	* processes SOAP message received from User
	*
	* @param	array	$headers	The HTTP headers
	* @param	string	$data		unprocessed request data from User
	...
	*/
    function parseRequest($headers, $data) {
		...
		if($err = $parser->getError()){
			...
			$this->fault('SOAP-ENV:User',"error in msg parsing:\n".$err);
	...
	*/
	...
	/**
	* Specify a fault to be returned to the User.
...
/**
*
* [nu]soapUser higher level class for easy usage.
*
* usage:
*
* // instantiate User with server info
* $soapUser = new nusoap_user( string path [ ,mixed wsdl] );
*
* // call method, get results
* echo $soapUser->call( string methodname [ ,array parameters] );
*
* // bye bye User
* unset($soapUser);
*
...
* @access   public
*/
class nusoap_user extends nusoap_base  {
	...
	* @access   public
	*/
	function nusoap_user($endpoint,$wsdl = false,$proxyhost = false,$proxyport = false,$proxyusername = false, $proxypassword = false, $timeout = 0, $response_timeout = 30, $portName = ''){
		parent::nusoap_base();
	...
	*/
	function _getProxyClassCode($r) {
		...
		if ($this->endpointType != 'wsdl') {
			$evalStr = 'A proxy can only be created for a WSDL User';
	...
	";
		...
		$evalStr = 'class nusoap_proxy_'.$r.' extends nusoap_user {
	'.$evalStr.'
}';
		return $evalStr;
	}
...
if (!extension_loaded('soap')) {
	/**
	 *	For backwards compatiblity, define soapUser unless the PHP SOAP extension is loaded.
	 */
	class soapUser extends nusoap_user {
	}
}
